...
import com.kidstune.video.centras.activity.Search_Intent_Activity;
...
public class SearchIntentAdapter extends BaseAdapter{
	...
	public SearchIntentAdapter(Context context, ArrayList<Main_Data> list, ListView listview_main) {
	...
	public View getView(final int position, View view, ViewGroup parent) {
		try{
			if(view == null){	
				...
				view = layoutInflater.inflate(R.layout.activity_intent_search_listrow, parent, false); 
			}
			...
			cursor = Search_Intent_Activity.favorite_mydb.getReadableDatabase().rawQuery(
			...
			bt_favorite.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(num.equals("empty")){
	    				...
	    				Search_Intent_Activity.favorite_mydb.getWritableDatabase().insert("favorite_list", null, cv);
						if(Search_Intent_Activity.adapter != null){
							Search_Intent_Activity.adapter.notifyDataSetChanged();
						}
						Toast.makeText(context, context.getString(R.string.txt_main_activity1), Toast.LENGTH_SHORT).show();
					}else{
						bt_favorite.setImageResource(R.drawable.bt_favorite_normal);
						Search_Intent_Activity.favorite_mydb.getWritableDatabase().delete("favorite_list", "num" + "=" +num, null);
						if(Search_Intent_Activity.adapter != null){
							Search_Intent_Activity.adapter.notifyDataSetChanged();
						}
			...
			Search_Intent_Activity.favorite_mydb.close();
			if(cursor != null){
				cursor.close();	
			}
		}
